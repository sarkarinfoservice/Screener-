...
# ==========================================
# APP LEVEL TABS (Direct Home Page Navigation)
# ==========================================
main_tab1, main_tab2 = st.tabs(["🔍 Stock Analysis", "🔥 Aaj Ke Trending Stocks"])

# ------------------------------------------
# MAIN TAB 1: STOCK ANALYSIS (Search Feature)
# ------------------------------------------
with main_tab1:
                ...
                try:
                    ...
                    if df.empty or len(df) < 2:
                        ...
                        with tab2:
                            ...
                            st.markdown("---")
                            swing_score = 0
                            if latest_close > latest_ema20: swing_score += 1
                            if 40 <= latest_rsi <= 65: swing_score += 1
                            if latest_macd > latest_signal: swing_score += 1
                            if latest_vol > vol_sma20: swing_score += 1
                        ...
                        with tab3:
                            ...
                            pb_str = f"{float(pb_ratio):.2f}" if pb_ratio and not pd.isna(pb_ratio) else 'N/A'
                            profit_margin = info.get('profitMargins', None)
                            pm_str = f"{float(profit_margin) * 100:.2f}%" if profit_margin and not pd.isna(profit_margin) else 'N/A'
                            
                            st.markdown("#### 📊 Core Fundamental Metrics")
                            m1, m2 = st.columns(2)
                            with m1:
                                st.metric("P/E Ratio (Valuation)", f"{pe_str}")
                                st.metric("Debt-to-Equity (Karza)", f"{de_str}")
                                st.metric("Net Profit Margin", f"{pm_str}")
                            with m2:
                                st.metric("ROE (Return on Equity)", f"{roe_str}")
                                st.metric("P/B Ratio (Book Value)", f"{pb_str}")
                                
                            lt_score = 0
                            if pe_val and pe_val < 25: lt_score += 1
                            if roe_val and roe_val > 15: lt_score += 1
                            if de_val and de_val < 0.5: lt_score += 1
...
# ------------------------------------------
# MAIN TAB 2: TRENDING STOCKS (Direct Access)
# ------------------------------------------
with main_tab2:
    ...
    with st.spinner("Market ka live data fetch ho raha hai..."):
    ...
    if not trending_df.empty:
        ...
        * **💼 Long-Term Nivesh Ke Liye:** Agar koi mazboot company (jaise Tata Motors, Reliance, HDFC Bank) yahan consolidation ya halke dip par dikhe, toh use 'Stock Analysis' wale tab mein search karke fundamentals check karein aur SIP ke liye chun sakte hain.
